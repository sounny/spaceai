...
// Babylon.js 3D Timeline with static cubes on arc
function initTimelineCubes3D() {
  ...
  // Fixed orthographic camera - completely static, no user control
  const camera = new BABYLON.OrthoCamera('camera', 30, 30, 0.1, 100, scene);
  ...
  // Build the arc path (flipped on x-axis)
  const arcPath = [];
  for (let t = 0; t <= 1; t += 0.05) {
    // Cubic Bezier curve flipped: start at right (25), dip up, end at left (-25)
    const x = -(Math.pow(1 - t, 3) * 0 + 3 * Math.pow(1 - t, 2) * t * 6.25 + 3 * (1 - t) * Math.pow(t, 2) * 18.75 + Math.pow(t, 3) * 25);
    const y = Math.pow(1 - t, 3) * 3.75 + 3 * Math.pow(1 - t, 2) * t * (-1.25) + 3 * (1 - t) * Math.pow(t, 2) * (-1.25) + Math.pow(t, 3) * 3.75;
    const z = 0;
    arcPath.push(new BABYLON.Vector3(x, y, z));
  }

  // Define cube data
  const cubes = [
    { pos: 0.05, label: 'Pilot study', description: 'Rwanda feasibility assessment' },
    { pos: 0.50, label: 'Model development', description: 'EVI & classification modeling' },
    { pos: 0.86, label: 'Scale-up', description: 'Policy integration & deployment' }
  ];

  // Store cube info for click detection
  const cubeData = [];

  // Create static 3D cubes positioned on the arc
  cubes.forEach((data, index) => {
    // Get position on the arc curve
    const arcIndex = Math.floor(data.pos * (arcPath.length - 1));
    const targetPos = arcPath[arcIndex];

    // Create cube mesh
    const cube = BABYLON.MeshBuilder.CreateBox('cube_' + index, { size: 1.2 }, scene);
    cube.position = targetPos.clone();

    // Create material
    const material = new BABYLON.StandardMaterial('mat_' + index, scene);
    material.diffuse = new BABYLON.Color3(0.1, 0.6, 1); // blue
    material.specularColor = new BABYLON.Color3(1, 1, 1);
    material.specularPower = 32;
    material.emissiveColor = new BABYLON.Color3(0.05, 0.3, 0.6);
    ...
    scene.registerBeforeRender(function () {
      time += 0.012;
      cube.rotation.x = Math.sin(time + index) * 0.25;
      cube.rotation.y = Math.cos(time + index) * 0.35;
      cube.rotation.z = Math.sin(time * 0.7 + index) * 0.15;
      cube.position.y = targetPos.y + Math.sin(time * 1.5) * 0.8;
    });
  });

  // Raycast for cube click detection
  canvas.addEventListener('click', (event) => {
    const rect = canvas.getBoundingClientRect();
    const x = event.clientX - rect.left;
    const y = event.clientY - rect.top;

    // Create a ray from camera through the clicked pixel
    const ray = BABYLON.Ray.CreateUsingRenderingMatrix(
    ...
    if (hit && hit.hit) {
      ...
      const cubeInfo = cubeData.find(c => c.mesh === hitMesh);
      if (cubeInfo) {
        console.log('Clicked:', cubeInfo.label, '-', cubeInfo.description);
        alert(cubeInfo.label + '\n' + cubeInfo.description);
      }
    }
  });
  ...
  canvas.addEventListener('mousemove', (event) => {
    ...
    if (hit && hit.hit) {
      hitCube = cubeData.find(c => c.mesh === hit.pickedMesh);
    }

    // Update all cubes: highlight hovered, dim others
    cubeData.forEach((cube) => {
      if (cube === hitCube) {
        cube.mesh.scaling = new BABYLON.Vector3(1.4, 1.4, 1.4);
        cube.material.emissiveColor = new BABYLON.Color3(0.4, 0.9, 1);
        canvas.style.cursor = 'pointer';
      } else {
        cube.mesh.scaling = cube.originalScale;
        cube.material.emissiveColor = cube.originalEmissive;
